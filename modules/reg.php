...
if (isset($_POST['reg'])
        AND isset($_POST['new_acc'])
        AND ($_POST['reg'] == '1')
        AND isset($_POST['pass1'])
        AND isset($_POST['pass1']))
    {
    $_POST['pass1'] = addslashes($_POST['pass1']);
    $_POST['pass2'] = addslashes($_POST['pass2']);
    $_POST['new_acc'] = addslashes($_POST['new_acc']);
    $_POST['email'] = addslashes($_POST['email']);
    $er = 0;
    $er_txt = '';
    if (!ValidationPasswOrName($_POST['new_acc']) || !ValidationFieldLen($_POST['new_acc']))
        {
        $er = 1;
        $er_txt = $er_txt . $txt['354'] . '<br>';
        }
    if (!ValidationEmail($_POST['email']))
        {
        $er = 1;
        $er_txt = $er_txt . $txt['50'] . '<br>';
        }
    if (($_POST['pass1'] == $_POST['new_acc']) || ($_POST['pass1'] != $_POST['pass2']) || !ValidationFieldLen($_POST['pass1']))
        {
        $er = 1;
        $er_txt = $er_txt . $txt['51'] . '<br>';
        }
    if (($_POST['pass1'] == '') OR ($_POST['pass2'] == '') OR ($_POST['new_acc'] == '') OR ($_POST['email'] == ''))
        {
        $er = 1;
        $er_txt = $er_txt . $txt['52'] . '<br>';
        }
    if ($KcaptchaEnable > 0)
        {
