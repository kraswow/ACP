...
if ($EventShow == 2) // Показ перечня всех ивентов
    {
    $equery = "SELECT `game_event`.`" . $eventrow . "` as entry,"
...
elseif ($EventShow == 3) // Ежедневные события
    ...
    $equery = "SELECT `game_event`.`" . $eventrow . "` as entry,"
            ...
            . "(`occurence` < 1441) ORDER BY `" . $eventrow . "`";
    $evres = mysql_query($equery, $m_connect);
    $ev_kol = mysql_num_rows($evres);
    if ($ev_kol > 0)
        {
    ...
    $equery = "SELECT `" . $eventrow . "` as entry,unix_timestamp(`start_time`) as start_time,"
            ...
            . ') AND ' . "(`occurence` > 1441) AND (`length` < 40300) order by `" . $eventrow . "`";
    $evres = mysql_query($equery, $m_connect);
...
// Начинаем рисовать календарик
    ...
    while ($day_count <= $dayofmonth) // Цикл на весь месяц.
        ...
        for ($i = 1; $i < 8; $i++)
            ...
            if ($day_count == 1 && $i < date('w', $unixtime) && date('w', $unixtime) <> 0)
                {
                echo '<img src="images/left.gif">'; // Если это день предыдущего месяца, то ничего не вставлять.
                } elseif ($day_count == 1 && date('w', $unixtime) == 0 && $i < 7) 
                {
                echo '<img src="images/left.gif">'; // Если это день предыдущего месяца с 1 днем в воскресенье, то ничего не вставлять.
...
if (isset($_GET['view']) && ((int) $_GET['view'] > 0 ))
    ...
    $equery = "SELECT `" . $eventrow . "` as entry,unix_timestamp(`start_time`) as start_time,"
            . "unix_timestamp(`end_time`) as end_time,`occurence`,`length`,"
            . "`holiday`,`description` FROM `game_event` WHERE "
            . "`" . $eventrow . "` = " . (int) $_GET['view'] . " LIMIT 1";
    $evres = mysql_query($equery, $m_connect);
    ...
    if ($ServerSystem == 1)
        {
        $equery = "SELECT count(`eventEntry`) FROM `game_event_gameobject` WHERE `eventEntry` = "
                . (int) $eres['entry'];
        } else
        {
    ...
    if ($ServerSystem == 1)
        {
        $equery = "SELECT count(`eventEntry`) FROM `game_event_creature` WHERE `eventEntry` = "
                . (int) $eres['entry'];
        } else
        {
        $equery = "SELECT count(`event`) FROM `game_event_creature` WHERE `event` = "
                . (int) $eres['entry'];
        }
    $evres = mysql_query($equery, $m_connect);
    $ores = mysql_fetch_array($evres);
    ...
    if ($ServerSystem == 1)
        {
        $equery = "SELECT count(`eventEntry`) FROM `game_event_quest` WHERE `eventEntry` = "
                . (int) $eres['entry'];
        } else
        {
        $equery = "SELECT count(`event`) FROM `game_event_quest` WHERE `event` = "
                . (int) $eres['entry'];
        }
    $evres = mysql_query($equery, $m_connect);
    $ores = mysql_fetch_array($evres);
