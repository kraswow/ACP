...
echo '<tr><td height="25" colspan="5" align="center" valign="middle" class="TableTitle"><b>'
 . $txt[108] . '</b></td></tr>';
echo '<tr><td width="10" height="40" class="TableLeft">&nbsp;</td>';
echo '<td width="200" height="40" class="TableCenter" align="center" valign="middle">'
 . $txt[109] . '<b>' . $_GET['id'] . '</b></td>';
...
mysql_query("DELETE FROM `mail` WHERE (UNIX_TIMESTAMP(`date`) + 60*60*24*3)< UNIX_TIMESTAMP() AND `mode` = 1");
...
if (mysql_num_rows($res) == 1)
    ...
    if ($row['mode'] == 1)
        ...
        mysql_select_db($r_db, $r_connect);
        mysql_query("SET NAMES 'utf8'");
        $r3 = mysql_fetch_assoc(mysql_query("select `username`,`sha_pass_hash`,`email` from `account`  WHERE `id` = "
                        . $row['account'] . " limit 1"));
        $new_pass = strtoupper(generate(10));
        $new_hash = SHA1(strtoupper($r3['username'] . ':' . $new_pass));
        ...
        require('include/log.php');
        // снимаем блокировку после восстановления пароля. при "живом" мыле.
        if (isset($r3['email']) && $r3['email'] <> '')
            {
    ...
    if ($row['mode'] == 2)
        {
        // Форма смена адреса электронной почты
        echo '<form method="post" action="?modul=mail&id=' . $_GET['id'] . '">';
        echo '<input name="modul" value="mail" type=hidden><input name="id" value="13" type=hidden>';
        echo '<table width="500" border="0" align="center" cellpadding="0" cellspacing="0">';
        echo '<tr><td height="25" colspan="3" align="center" valign="middle" class="TableTitle">'
        . $txt[7] . '</td></tr>';
        echo '<tr><td width="300" align="right" height="60" valign="middle" class="TableLeft">'
        . $txt[113] . '</td>';
        echo '<td width="10" height="60" valign="middle" class="TableCenter">&nbsp;</td>';
        echo '<td width="190" height="60" align="left" valign="middle" class="TableRight">';
        echo '<input name="new_email" type="text" /></td></tr>';
        echo '<tr><td width="300" align="right" valign="middle">&nbsp;</td>';
        echo '<td width="10" height="40" valign="middle">&nbsp;</td>';
        echo '<td width="190" height="40" align="left" valign="middle"><input type="submit" value="'
        . $txt[11] . '"></td>';
        echo '</tr>';
        echo '</table></form>';
        return;
    ...
    if ($row['mode'] == 4)
        {
        // снятие блокировки при заполнении емайла
        $r_connect = mysql_connect($r_ip, $r_userdb, $r_pw);
        mysql_select_db($r_db, $r_connect);
        mysql_query("SET NAMES 'utf8'");
    ...
    echo '<br>';
    echo '<table width="500" border="0" cellspacing="0" cellpadding="0">';
    echo '<tr><td height="25" align="center" valign="middle" class="ErrTitle">';
    echo '<b>' . $txt['20'] . '</b></td></tr>';
    echo '<tr><td height="45" align="center" valign="middle"  class="ErrTab"><b>'
    . $txt['110'] . '</b></td></tr></table><br>';
    }
?>