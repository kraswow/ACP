...
if (isset($ModulesView) and ($ModulesView == 1))
    ...
    if (isset($_GET['modul'])
            AND isset($modules[0][$_GET['modul']])
            AND ($modulesAccess[$_GET['modul']] == 0))
        $MenuActiveModul = $_GET['modul'];
    ...
    while (list($menu1, $menu2) = each($modules))
        ...
        if ($menuShow == 1)
            ...
            if ($MenuActiveModul == $menu1)
                {
                echo '<option value="' . $menu1 . '" selected>' . $txt[$menu2[1]] . '</option>' . "/n";
                $sel = 1;
                }
            else
                echo '<option value="' . $menu1 . '">' . $txt[$menu2[1]] . '</option>' . "/n";
            }
        }
    if ($sel == 0)
...
elseif (isset($ModulesView) and ($ModulesView == 2))
    ...
    while (list($menu1, $menu2) = each($modules))
        ...
        if (($menu2[2] < 0) and ($menu2[4] == 1))
            {
            $MenuForAll = $MenuForAll . "\n" . '<tr><td align="left" valign="middle"><a href="index.php?modul=' . $menu1 . '" target=_self>' . $txt[$menu2[1]] . '</a></td></tr>';
